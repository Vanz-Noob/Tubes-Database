<?php include('config.php'); ?>
		...
		if(isset($_POST['submit'])){
			...
			if(mysqli_num_rows($cek) == 0){
				$sql = mysqli_query($koneksi, "INSERT INTO mahasiswa(Nim, Nama_Mahasiswa, Jenis_Kelamin, Id_Program_Studi, Kode_dosen, Status) VALUES('$NIM', '$NAMA_Mahasiswa', '$JENIS_KELAMIN', '$ID_PROGRAM_STUDI', '$KODE_DOSEN', '$STATUS')") or die(mysqli_error($koneksi));
			...
			<div class="item form-group">
				...
				<div class="col-md-6 col-sm-6">
					...
					<option value="">Pilih Program Studi</option>
						...
						<option value="BM">Biomedis</option>
					</select>
				</div>
			</div>
			<div class="item form-group">
				<label class="col-form-label col-md-3 col-sm-3 label-align">ID Dosen</label>
				<div class="col-md-6 col-sm-6">
					<select name="Kode_dosen" class="form-control" required>
						<option value="">Pilih ID dosen sesuai dengan wali dosen Anda</option>
						<option value="RJO">Raharjo(RJO)</option>
						<option value="REN">Renaldo(REN)</option>
					</select>
				</div>
			</div>	
			<div class="item form-group">
				...
				<div class="col-md-6 col-sm-6 ">
					<div class="btn-group" data-toggle="buttons">
						<label class="btn btn-secondary" data-toggle-class="btn-primary" data-toggle-passive-class="btn-default">
							<input type="radio" class="join-btn" name="Status" value="Aktif" required>Aktif
						</label>
						<label class="btn btn-primary " data-toggle-class="btn-primary" data-toggle-passive-class="btn-default">
							<input type="radio" class="join-btn" name="Status" value="Lulus" required>Lulus
						</label>
						<label class="btn btn-danger " data-toggle-class="btn-primary" data-toggle-passive-class="btn-default">
							<input type="radio" class="join-btn" name="Status" value="DO" required>DO
						</label>
					</div>
				</div>
			</div>
